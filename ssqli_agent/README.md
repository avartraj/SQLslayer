...
## Architecture
This folder is **only the tool**. The vulnerable demo API and the test suite
live in sibling folders (`../vulnerable_target`, `../tests`) so the scanner stays
fully decoupled from anything it scans.
```
...
../vulnerable_target/           # Separate, standalone vulnerable Flask API (a target)
../tests/                       # Separate unit tests + demo scenario runner
```
...
[agent/llm_providers.py](agent/llm_providers.py) (the registry / "hook"). Edit
that file to add a provider or change a default model.

```powershell
python main.py --list-models                         # show all providers + models
...
```powershell
python main.py --url "https://demo.test/item?id=1&cat=books"
```
...
### Diagnostics
```powershell
python main.py --list-payloads     # payload library summary
```

## Reports
Every scan writes three reports to `reports/`, all in a formal **penetration-test
format**, and each finding is labelled with its exact **SQLi type** (UNION-based,
Boolean-blind, Time-based, Error-based, Stacked, Auth-bypass, …):
...
## Demo & tests (sibling folders)
```powershell
python ..\tests\test_runner.py        # launch vulnerable target + full scenario scan
python ..\tests\test_agent_unit.py    # tool unit tests
python ..\tests\test_recon_unit.py    # recon pipeline unit tests
python ..\vulnerable_target\app.py    # run the vulnerable target on its own
```
...
- **Similarity-ratio comparison** ([response_compare.py](utils/../agent/response_compare.py)) —
...
- **DBMS fingerprinting** ([dbms_fingerprint.py](agent/dbms_fingerprint.py)) —
...
Relevant `AgentConfig` knobs (in [config.py](config.py)):
| Flag | Default | Effect |
|------|---------|--------|
